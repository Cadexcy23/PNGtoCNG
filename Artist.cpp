...
//Screen constants    
int Artist::SCREEN_WIDTH = 160;
int Artist::SCREEN_HEIGHT = 50;
...
void convertPNGtoCNG()
{
	SDL_Surface* png = IMG_Load("png.png");

	int bpp = png->format->BytesPerPixel;
	...
	if (myFile.is_open())
	{
		...
		//go thru each pixel of the png
		for (int y = 0; y < png->h; y++)
		{
			for (int x = 0; x < png->w; x++)
			{
				uint32_t* p = ((uint32_t*)png->pixels + y * png->w + x);
				SDL_GetRGB(*p, png->format, &red, &green, &blue);
				
				

				//float redR = rand() % 255;
				//float greenR = rand() % 255;
				//float blueR = rand() % 255;
				//if (red == 255)
				//{
				//	printf("pog");
				//}
				...
				float index = 0;
				int colorDif = abs(red - green) + abs(red - blue) + abs(green - blue);

				if (colorDif < 10 && red > 0 && green > 0 && blue > 0 && red < 255 && green < 255 && blue < 255)
				{
					float averageColor = (red + blue + green) / 3;
					if (averageColor > 150)
					{
						printf("pog");
					}
					index = 240 + round(averageColor / 255 * 15);
				}
				else
				{
					index += round(float(red) / 255 * 5); //round these instead of truncate
					index += round(float(blue) / 255 * 4) * 6;
					index += round(float(green) / 255 * 7) * 30;
				}
				...
				myFile << std::hex << (int)index;
				
				//if (red < 16)
				//{
				//	myFile << 0;
				//	myFile << std::hex << (int)red;
				//}
				//else
				//{
				//	myFile << std::hex << (int)red;
...
void Artist::draw()
{
	...
	pngToConvert = artist.loadTexture("png.png");


	
	
	//use draw functions from other classes here \/
	artist.drawImage(pngToConvert, 0, 0);
